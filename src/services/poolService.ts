...
export async function assignRideToPool(rideRequestId: number) {
  return prisma.$transaction(async (tx) => {
    ...
    for (const pool of openPools) {
      ...
      if (occupiedSeats + ride.seats_requested <= pool.vehicle.capacity) {
        const pooledFare = calculateEstimatedFare(true);

        // This ride joins as a pooled member.
        await tx.poolMember.create({
          data: { pool_id: pool.id, ride_request_id: ride.id, fare: pooledFare },
        });

        const updatedRide = await tx.rideRequest.update({
          where: { id: ride.id },
          data: { status: "MATCHED", estimated_fare: pooledFare },
        });

        // A pool now has 2+ members — recalculate EVERY existing member's
        // fare to the pooled rate too, so nobody is left paying the earlier
        // solo-rate fare just because they joined before anyone else did.
        for (const member of freshMembers) {
          if (member.fare !== pooledFare) {
            await tx.poolMember.update({
              where: { id: member.id },
              data: { fare: pooledFare },
            });
            await tx.rideRequest.update({
              where: { id: member.ride_request_id },
              data: { estimated_fare: pooledFare },
            });
          }
        }
    ...
    // First (and so far only) member of a brand-new pool pays solo rate,
    // since there's no one to share the discount with yet. If someone
    // else joins later, both fares get recalculated in the branch above.
    const soloFare = calculateEstimatedFare(false);

    await tx.poolMember.create({
      data: { pool_id: newPool.id, ride_request_id: ride.id, fare: soloFare },
    });

    const updatedRide = await tx.rideRequest.update({
      where: { id: ride.id },
      data: { status: "MATCHED", estimated_fare: soloFare },
    });
